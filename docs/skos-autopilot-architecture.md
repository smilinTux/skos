...
**v1.5 supersedes this section's earlier bwrap design with a Docker sandbox.** See the full design and build plan: `docs/superpowers/specs/2026-07-13-autopilot-v15-sovereign-sandbox-design.md` and `docs/superpowers/plans/2026-07-13-autopilot-v15-sandbox.md`. The rest of this section describes the shipped v1.5 model.

**v1 posture (operator decision 2026-07-12, after a security review found the v1 wrapper was composed but never applied at spawn): the harness is HARD-DISABLED from live execution in v1.** `ClaudeCodeAdapter._spawn` raises `HarnessUnavailable` unless `live_execution=True`, which v1 never sets; v1 runs the orchestration, routing, digest, and coord-scoring machinery in dry-run against a deterministic stub harness, never spawning a real model process. `live_execution` stays config-gated (default false) in v1.5 too: the confinement proof (`RUN_SANDBOX_IT=1`) must pass on a node before it is enabled there. The firewall (below) IS enforced now: `is_forbidden` normalizes input, rejects whole-server `mcp__<server>` grants, and denies outbound-comms/secret tools by category, and the constructor fails closed if the allowlist contains any of them.
...
- **Docker sandbox confinement (closes the injection to secret-read to exfil chain), replacing bwrap.** bwrap is not viable rootless on noroc2027 (uid_map permission denied), so confinement is a disposable Docker container instead. Secrets are confined by ABSENCE, not by a path denylist: the container's HOME is the image's own, so `~/.skcapstone`, skvault, `~/.hermes`, and `~/.ssh` simply do not exist inside it. Only the task worktree (read-write, mounted at `/work`) and the single harness auth credential (read-only) are mounted in; the docker socket is never mounted, so a compromised session cannot reach the host's container engine. Hardening on top of that: `--read-only` root filesystem, `--cap-drop ALL`, `--security-opt no-new-privileges`, a non-root user, `--pids-limit`, and resource caps (memory/CPU). If the sandbox images are missing on a given node, engineering execution on that node is disabled rather than run unconfined (fail closed).
- **Egress via a sovereign allowlist proxy sidecar.** The container sits on an internal Docker network whose sole route out is an allowlist proxy sidecar (`sandbox_proxy.py`), permitting only the repo's git remote host, the CI host, and the harness inference host (skgateway for a local model). The proxy handles both HTTPS `CONNECT` tunnels and plain-HTTP forwarding (the latter so a container can reach the local http skgateway through it); for a forwarded HTTP response it buffers and de-chunks the body, so it strips the upstream's `Transfer-Encoding: chunked` / `Content-Length` framing and sets an explicit `Content-Length` (without this, a strict client such as opencode/undici rejects the reply as `InvalidHTTPResponse` on streaming endpoints). Arbitrary git remotes and all other outbound are denied, so an allowed push cannot be redirected to an attacker remote and there is no general exfil channel. With a local-model harness (pi or opencode routed through skgateway), all egress stays on the tailnet, so there is no public-internet leg at all for the inference channel. This resolves former Open Decision 5: egress is an enforced control, not a compensating one. Note one inherent residual: the inference channel itself is a named, reviewed entry in the egress allowlist (not an implicit passthrough), and the data-framing rule plus the denial of all outbound-comms tools bound the exfil-via-inference risk inherent to any LLM agent.
- **The harness is a swappable registry, all three real harnesses proven live.** `build_harness(name, ...)` (selected by `autopilot.yaml: harness.name` or `--harness`) builds one of: `pi` (the sovereign target), `claude-code` (the v1 default), `opencode`, or `codex-stub`. Each is an adapter module over the shared `Sandbox`, so the confinement, mount, and egress model above is common to every harness and a harness swap changes only which process runs inside the container. Build the images with `docker/sandbox/build.sh` (all four, or a named subset); a node missing them fails closed. Verified end-to-end live in the confined sandbox: **claude-code** (full executor + orchestrator canary on the operator's Max OAuth, no per-token API key injected); **pi** routed to a local model via skgateway (one turn, terminates cleanly in ~3.6s, the cleanest sovereign harness, no timeout cap needed); **opencode** routed to skgateway (functional and correct, but it agent-loops past its first answer even with tools off, so the adapter bounds its `run_timeout` (300s default) and `Sandbox.spawn` preserves partial stdout on timeout so `_parse` recovers the first-chunk answer). pi and opencode both route through the injected provider config (`config_files`): pi via `/agent/models.json` + `PI_CODING_AGENT_DIR`, opencode via `/cfg/opencode.json` (`OPENCODE_CONFIG`), authenticated by an in-config api key, not a mounted credential.
