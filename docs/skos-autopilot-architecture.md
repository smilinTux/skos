...
**v1 posture (operator decision 2026-07-12, after a security review found the v1 wrapper was composed but never applied at spawn): the harness is HARD-DISABLED from live execution in v1.** `ClaudeCodeAdapter._spawn` raises `HarnessUnavailable` unless `live_execution=True`, which v1 never sets; v1 runs the orchestration, routing, digest, and coord-scoring machinery in dry-run against a deterministic stub harness, never spawning a real model process. The controls below (confined Bash, pinned egress) are composed and unit-tested but are not trusted until the **v1.5 sovereign-sandbox build** (coord `07c78c7f`) completes them: wire the wrapper into the spawn (done in v1), bind claude auth read-only into the jail while HOME stays tmpfs-hidden, replace `--share-net` with `--unshare-net` plus a pinned egress proxy that enforces the allowlist, and prove via an integration test that a confined session cannot read `~/.skcapstone`/skvault and cannot reach an off-allowlist host. Only after that may `live_execution` be enabled and canary/auto-merge considered. The firewall (below) IS enforced now: `is_forbidden` normalizes input, rejects whole-server `mcp__<server>` grants, and denies outbound-comms/secret tools by category, and the constructor fails closed if the allowlist contains any of them.
