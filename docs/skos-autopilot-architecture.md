...
- **Egress via a sovereign allowlist proxy sidecar.** The container sits on an internal Docker network whose sole route out is an allowlist CONNECT proxy sidecar, permitting only the repo's git remote host, the CI host, and the harness inference host (skgateway for a local model). Arbitrary git remotes and all other outbound are denied, so an allowed push cannot be redirected to an attacker remote and there is no general exfil channel. With a local-model harness (for example pi or opencode routed through skgateway), all egress stays on the tailnet, so there is no public-internet leg at all for the inference channel. This resolves former Open Decision 5: egress is an enforced control, not a compensating one. Note one inherent residual: the inference channel itself is a named, reviewed entry in the egress allowlist (not an implicit passthrough), and the data-framing rule plus the denial of all outbound-comms tools bound the exfil-via-inference risk that is inherent to any LLM agent.
- **The harness is a swappable registry.** `build_harness(name, ...)` (selected by `autopilot.yaml: harness.name` or `--harness`) builds one of: `pi` (the sovereign target), `claude-code` (the v1 default), `opencode`, or `codex-stub`. Each is an adapter module over the shared `Sandbox`, so the confinement, mount, and egress model above is common to every harness and a harness swap changes only which process runs inside the container.
