...
@app.command("sync")
def sync(
    canon: Path | None = typer.Option(None),
    ...
    with PostgresWriterBackend(dsn_from_env("SKBRAIN_PG_PROJECTOR_DSN")) as backend:
        ...
        result = project(
            pages, backend, embedder=MxbaiEmbedder() if commit else None, commit=commit
        )
...
@app.command("search")
def search(
    query: str, limit: int = typer.Option(8, min=1, max=25), kind: str | None = None
) -> None:
...
def doctor(canon: Path | None = typer.Option(None)) -> None:
    ...
    typer.echo(
        json.dumps(
            {"ok": all(c.ok for c in checks), "checks": checks_json(checks)}, sort_keys=True
        )
    )
...
def operator_observe(
    ...
    def condition(name: str, checks_: tuple[str, ...]) -> dict[str, object]:
        ...
        return {
            "type": name,
            "status": "True" if relevant and all(x.ok for x in relevant) else "Unknown",
